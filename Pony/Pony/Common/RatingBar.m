...
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<RatingBarDelegate>)delegate{
    ...
    if (height < [fullSelectedImage size].height) {
        height = [fullSelectedImage size].height * 1.5;
    }
    if (height < [halfSelectedImage size].height) {
        height = [halfSelectedImage size].height * 1.5;
    }
    if (height < [unSelectedImage size].height) {
        height = [unSelectedImage size].height * 1.5;
    }
    if (width < [fullSelectedImage size].width) {
        width = [fullSelectedImage size].width * 1.5;
    }
    if (width < [halfSelectedImage size].width) {
        width = [halfSelectedImage size].width * 1.5;
    }
    if (width < [unSelectedImage size].width) {
        width = [unSelectedImage size].width * 1.5;
    }
    
    float gap= (250.0 - width * 5.0)/2;
    ...
    [_s1 setFrame:CGRectMake(gap,         0, width, height)];
    [_s2 setFrame:CGRectMake(gap + width,     0, width, height)];
    [_s3 setFrame:CGRectMake(gap + 2 * width, 0, width, height)];
    [_s4 setFrame:CGRectMake(gap + 3 * width, 0, width, height)];
    [_s5 setFrame:CGRectMake(gap + 4 * width, 0, width, height)];
